...
class NeuralNet:
    ...
    def backProp(self, X, Y, dropProb):
        ...
        delta[-1] = np.multiply(a[-1] - Y, n)  # this is an n-dim array
    ...
    def train(self, X, Y, iters, alpha, regParam=0, errors=False, verbose=False, 
        silent=False, dropProb=0, batchSize=0, b1=0, b2=0, tX = [], tY = [], printGap=10):


        m, n = X.shape

        c = alpha # multiplication constant
        ...
        sW = [np.zeros(layer.W.shape) for layer in self.network]
        sb = [np.zeros(layer.b.shape) for layer in self.network]

        rW = [np.zeros(layer.W.shape) for layer in self.network]
        rb = [np.zeros(layer.b.shape) for layer in self.network]
        ...
        for iter in range(iters):
            ...
            for x, y in zip(batches, labels):
                ...
                sW = np.multiply(sW, b1) + np.multiply(tempDW, 1 - b1)
                sb = np.multiply(sb, b1) + np.multiply(tempDb, 1 - b1)

                rW = np.multiply(rW, b2) + np.multiply(np.multiply(tempDW, tempDW), 1 - b2)
                rb = np.multiply(rb, b2) + np.multiply(np.multiply(tempDb, tempDb), 1 - b2)

                momentumTime = time.time()
                #print("Took", momentumTime-before, "to acccount for momentum")

                sWHat = np.multiply(sW, 1.0 / (1 - b1 ** t))
                sBHat = np.multiply(sb, 1.0 / (1 - b1 ** t))

                rWhat = np.multiply(rW, 1.0 / (1 - b2 ** t))
                rBhat = np.multiply(rb, 1.0 / (1 - b2 ** t))

                for l in range(len(self.network)):
                    self.network[l].W = self.network[l].W * (1 - regParam*c) - c * sWHat[l] / (np.sqrt(rWhat[l]) + stabilizer)

                    self.network[l].b -= c * sBHat[l] / (np.sqrt(rBhat[l]) + stabilizer) 
